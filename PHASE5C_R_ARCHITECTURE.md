...
Proxy may reject missing origin/signature cheaply, but the mutation handler independently verifies the envelope against its actual method, URL, headers, byte count, and body digest before schema-dependent provider work. Never trust a client-set `body-verified`/`client-key` internal header. Explicitly preserve same-origin CSRF protection: exact Origin validation and the existing submission-token checks must replace the framework protection lost by moving out of a Server Action.

The installed `proxyClientMaxBodySize` guide warns that overflow truncates buffering and **does not reject the request**. It is not an enforcement gate. Keep its buffering threshold strictly above the handler's maximum (for example 64 KiB versus 32 KiB), and require the handler to reject as soon as byte 32,769 is observed. Setting the truncation threshold to exactly the accepted maximum could hide an appended tail behind an otherwise valid signed prefix. Signed lengths/digests, overflow-byte tests, and real-path tests must reject truncation; a framework path that silently hides overflow is not qualified. Preserve admin Server Actions and their independent authorization.
