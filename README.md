...
After validation, an enabled production path must pass independent budgets around Turnstile. The pre-verification operation permits 30 attempts per opaque client key in ten minutes and 300 globally per minute; verified requests then encounter the existing strict limits of five per key in ten minutes and 100 globally per minute. Each operation must enforce both rolling windows atomically in shared storage. Rejected challenges consume only the pre-verification budget. Either limiter failure denies persistence. The five-per-key policy is a retained baseline, not a new office-friendly go-live recommendation: shared-office measurement and capacity review remain required before enabling persistence. Public responses collapse failures into the neutral unavailable response without exposing keys, thresholds or providers. The memory adapter under `tests/support` is test-only.
...
Phase 5C reviewed current official Cloudflare/Vercel and Upstash guarantees. Proxied visitor identity and strict limiter consistency across failover remain unsupported; `cloudflare-via-vercel` and `upstash` cannot open intake. See the [security findings and adversarial verification](PHASE5C_SECURITY_REPORT.md) and [conditional provider smoke runbook](PHASE5C_PROVIDER_SMOKE_RUNBOOK.md). Do not enable persistence on this basis.

Client identity supports direct Vercel ingress only (`VERCEL=1`, `VERCEL_ENV=production`, and `SUBMISSION_CLIENT_IP_SOURCE=vercel`). Preview, development and unknown sources fail closed. Only one valid `x-vercel-forwarded-for` IPv4/IPv6 value is accepted; IPv6 spelling is normalized. Arbitrary forwarding alternatives are ignored; multi-hop/invalid platform values and unexpected Cloudflare/origin-secret headers deny identity derivation. These headers can trigger denial, never establish trust. The address becomes an HMAC-SHA-256 key inside the identity module and is not stored, returned, logged or sent to Turnstile. Cloudflare-derived identity is deliberately unsupported. `PUBLIC_ORIGIN_PROTECTION=required` independently closes public persistence even if a future provider is registered. See the [Phase 5A report](PHASE5A_SECURITY_REPORT.md) and [manual edge runbook](PHASE5A_EDGE_RUNBOOK.md).
