...
import { validateRenderedOperatorExecutorConfig } from "../deployment/operator-executor-contract";
...
async function main(): Promise<void> {
  ...
  validateRenderedOperatorExecutorConfig(config);
  const expectedMain = await realpath(join(root, "workers", "operator-lifecycle-executor.ts"));
  const resolvedMain = await realpath(resolve(dirname(configPath), "../workers/operator-lifecycle-executor.ts"));
  if (resolvedMain !== expectedMain) throw new Error("executor-main-mismatch");
  process.stdout.write("Private operator executor preflight: PASS (local validation only; no deployment).\n");
}

main().catch(() => { process.stderr.write("Private operator executor preflight: REFUSED.\n"); process.exitCode = 1; });
