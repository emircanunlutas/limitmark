...
import { MAX_SEALED_ARTIFACT_BYTES, parseSealedLifecycleArtifact, parseStrictJson } from "../operator/lifecycle-submitter";
...
async function manifest(): Promise<Manifest> {
  // This fixed staging path is the only target. The unresolved template is refused.
  let bytes: Uint8Array;
  try { bytes = await boundedFile("deployment/lifecycle-transport.staging.json", 2_048); }
